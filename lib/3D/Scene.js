	...
	componentDidMount(){
		...
		var init = function(){
			...
			var getSelectedObject = function( event , all = false ) {
			  ...
			  var intersects = raycaster.intersectObjects(global['scene_'+_this.state.scene_i].scene.children);
			  if (all==true) {
			  	return intersects
			  }
			...
			var removeObj = function(){
				var LineSegments = global['scene_'+_this.state.scene_i].scene.children.filter((item)=>{
					if (item.type=='LineSegments') {
						if (item.geometry.type=='EdgesGeometry') {
							global['scene_'+_this.state.scene_i].scene.remove(item)
							return true
						}
					}
				})
				...
				LineSegments.forEach((item)=>{
					var obj = global['scene_'+_this.state.scene_i].scene.children.find((_item)=>{
						if (_item.uuid==_this.state.plane_uuid) {return false}
						if (_item.position.distanceTo(item.position)==0) {
							return _item
						}
					})
					global['scene_'+_this.state.scene_i].scene.remove(obj)
					global['scene_'+_this.state.scene_i].scene.remove(item)
				})
			}
			var keyboardCtrl = function(event){
				...
				if (event.key=='`') {
					if (_this.state.editMode==false) {
						...
						global['scene_'+_this.state.scene_i].scene.children.forEach((item)=>{
							if (item.type=='LineSegments') {
								if (item.geometry.type=='EdgesGeometry') {
									global['scene_'+_this.state.scene_i].scene.remove(item)
								}
			...
			var changeObjState = function(clickedObject){
				var data = clickedObject.makeObjData
				if (data.alterId == data.alternative.length) {
					var id = 0
					var mesh = clickedObject.originMesh
					mesh.makeObjData.alterId = id
					global['scene_'+_this.state.scene_i].scene.remove(clickedObject)
				    mesh.uuid = clickedObject.uuid
	                ...
	                if (data.alternative[id-1].material) {
	                	if (data.alternative[id-1].material.type=='loadTexture') {
	                		...
	                		MAKEOBJ.loadTexture(data.alternative[id-1].material.url,function(texture){
			                	...
			                	mesh.animation = clickedObject.animation
				                mesh.originMesh = clickedObject.originMesh
				                global['scene_'+_this.state.scene_i].scene.remove(clickedObject)
				                mesh.uuid = clickedObject.uuid
				                global['scene_'+_this.state.scene_i].scene.add(mesh)
	                		})
	                	...
	                	var material = clickedObject.material
	                	var mesh = new THREE.Mesh(geometry,material)
		                if (data.alternative[id-1].translate) {
            				mesh.position.set(clickedObject.position.x+data.alternative[id-1].translate.x,clickedObject.position.y+data.alternative[id-1].translate.y,clickedObject.position.z+data.alternative[id-1].translate.z)
            			}
			...
			var clickObj = function(event){
				...
				if (clickedObject.uuid == _this.state.plane_uuid) {
					return
				}
				...
				if (_this.state.editMode==false&&event.button==0) {//非编辑模式下，转动视角，鼠标移动
					...
					var addEdges = function(event){//添加轮廓线
						if (getSelectedObject(event)) {
							...
							global['scene_'+_this.state.scene_i].scene.add(line)
						}
					}
					removeEdges()
					addEdges(event)
			...
			_this.canvasDOM.addEventListener('mousewheel',function(event){
				...
				if (event.wheelDeltaY>0) {//鼠标轮滑向下
					_this.zoomIn(global['scene_'+_this.state.scene_i].scene,_camera)
				}else{//鼠标轮滑向上
					_this.zoomOut(global['scene_'+_this.state.scene_i].scene,_camera)
				}
			})
