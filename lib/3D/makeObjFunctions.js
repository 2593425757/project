...
var turnToOrigin = function(mesh){
	if (!mesh.makeObjData) {return}
...
var importScene = function(sceneData,i='0'){
	...
	var makeObj = function(meshJson,Target){
		var {position,rotation,scale,originMesh,animation,makeObjData,children,uuid} = meshJson
		if (!makeObjData) {
			...
			getPromise.then(value=>{
				...
				for(let i=0;i<makeObjData.alterId;i++){
					changeState(mesh)
				}
